...
import { 
  CreditCard, Calendar, Plus, User, Filter, Search, Check, AlertCircle 
} from 'lucide-react';
import { Card, CardHeader, CardTitle, CardContent } from '../components/ui/Card';

const RentTrackingPage: React.FC = () => {
  const [rentPayments, setRentPayments] = useState<any[]>([]);
  const [tenants, setTenants] = useState<any[]>([]);
  const [pendingRent, setPendingRent] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Calculate tenants who haven't paid rent for the month
      const paidTenantIds = rentData?.map(payment => payment.tenants.id) || [];
      const pendingTenants = tenantsData?.filter(tenant => !paidTenantIds.includes(tenant.id)) || [];
      
      setPendingRent(pendingTenants);
    } catch (error) {
  ...
  const handleTenantChange = (id: string) => {
    ...
    setFormData({
      ...formData,
      tenant_id: id,
      amount_paid: tenant?.rooms.rent || 0
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Rent Tracking</h1>
      ...
      {showAddForm && (
        <Card className="border-blue-200">
          <CardHeader className="bg-blue-50">
            <CardTitle>Add Rent Payment</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleAddRentPayment} className="space-y-4">
                  ...
                  >
                    ...
                    {tenants.map((tenant) => (
                      <option key={tenant.id} value={tenant.id}>
                        {tenant.name} - Room: {tenant.rooms.name} (₹{tenant.rooms.rent}/month)
                      </option>
                    ))}
                  </select>
                </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h2 className="text-sm font-medium text-gray-500">Total Rent Collected</h2>
                <p className="text-2xl font-semibold text-gray-800">
                  ₹{rentPayments.reduce((sum, payment) => sum + payment.amount_paid, 0)}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-800">{rentPayments.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-800">{pendingRent.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {pendingRent.map((tenant) => (
                    <tr key={tenant.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{tenant.rooms.name}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">₹{tenant.rooms.rent}</div>
                      </td>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            ...
            Rent Payments for {format(new Date(selectedMonth + '-01'), 'MMMM yyyy')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredRentPayments.map((payment) => (
                    <tr key={payment.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">₹{payment.amount_paid}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link 
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500">
                {searchQuery ? 'Try a different search term' : `No rent payments recorded for ${format(new Date(selectedMonth + '-01'), 'MMMM yyyy')}`}
              </p>
              {!searchQuery && (
                <div className="mt-6">
                  <button
